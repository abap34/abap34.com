...
つまり、 各 $n$, $k$ に対して中心差分と同様に微分係数を求めたい点から左右に $h$ ずつ幅をとって点を取り評価した $n + 1$点に対して適切な重み付き和が存在して
...
したがって、 $\boldsymbol{w} = A^{-1}\boldsymbol{e}_{k+1}$ なる $\boldsymbol{w}$ がただひとつ存在して条件を満たす。
:::
...
(とはいえ、中心差分よりも正確にやろうとする場面すらなかなかみないので、ほとんど実用的ではないですが)
