...
例えば completion などはプログラムのある位置で存在しうる変数名の解析が必要ですし，
型推論などもプログラム解析の一種と言えると思います．


プログラム解析とひとくちに言っても膨大な種類がありますが，
これを **抽象解釈 (abstract interpretation)** という枠組みによって包摂的に捉えることができることが知られています．
(いろんな最適化が実はこういう線形計画問題になりますよ，というようなのと似た感じです)
...
1. **これを使ってさまざまな実践的なプログラム解析の枠組みを簡単に構成できること**を紹介します．
2. Julia コンパイラを例に，実践的な抽象解釈の実装設計との関連についても考えてみます．
3. Lean 4 による形式化にも挑戦してみます．
  [mathlib にはすでに Galois 接続のさまざまな性質が収録されている](https://leanprover-community.github.io/mathlib4_docs/Mathlib/Order/GaloisConnection/Defs.html) のですが， 勉強も兼ねてコアの部分はなるべく使わず自分で書くことにします． 大抵の定義や証明に切り替えボタンを置いておきます．押すと Lean のコードが見られます．


:::warn
**⚠️** 書いている人は専門家ではないので，誤りが含まれる可能性があります．
...
という感じのプログラムを実行したときに最後の行で `z` がとりうる値に関する何らかの解析を行うことを考えてみます．


最も理想的，具体的な解析結果は **「`z` がとりうる値は `0` または `2` ですよ」** ということでしょう．


しかし，このような最も具体的な解析は常にリーズナブルに可能，あるいはそもそも計算可能であるとは限りません．
そのため，多くのプログラム解析は「いい感じの」ところまで抽象化された値を求めることになります．


例えば，

- これが実は何らかの制御のプログラムで， `z` がとりうる最大値と最小値だけに関心があるので区間という形で解析する

というようなことです．


**プログラム解析とは，このような抽象化された「評価」を考えることといえます．**


つまり
- 最も具体的なもの (典型的には値の集合) と
- 所望の抽象的なもの (区間，型など)

を対応させていくことが必要になります．
**この対応のうち，望ましい性質を持ったものが Galois 接続です．**
...
がごちゃ混ぜに書かれていることに気がついた人もいるかもしれませんが，
この「評価の抽象化」は「値の抽象化」のみから構成できることも示していきます．

:::info

- 混同のおそれがないとき，半順序集合 $(L, \leq_L)$ の $\leq_L$ を省略して， $L$ と書いて半順序集合を表すことがあります．
- とくに断りがないとき，関数は全域です．
...
いくらか同値な定義 (この後紹介します) がありますが，今回はとりあえずこれを使います．
...
結論から言うとこのような疑問は正しく，次のようなことがいえます．
...
定義 1 の主張はざっくり図に起こせばこういうことになります．

(ここに手書きの図を載せる)

定義 2 も次のような対応と解釈できます．

(ここに手書きの図を載せる)


すると， $\gamma$ の行き先は， $\alpha$ の逆像の上限に他ならないことがわかります．
...
さて，先ほどの直感は Galois 接続がプログラム解析の適切な抽象化であることを前提に語っていましたが，
そのことの補強をしてみます．


Galois 接続がプログラム解析の適切な抽象化であることを確認するために，ここでは Galois 接続が「正当さ」を保つことを確認します．
