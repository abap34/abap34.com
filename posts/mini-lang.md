...
これにたいして PEG では順序付きの選択を使います。

```plaintext
A = B / C
```

と書いたとき、 `A` は `B` にマッチするか試し、成功したら `B` にマッチし、そして失敗したその時に限り `C` にマッチします。

したがって結果は必ず一意に定まります。

さらに、これはプログラムとの対応に思いを馳せると割と自然で、
選言がプログラムでの `||` に対応してほしいという気持ちになると、
`(f() > 0) || (g() > 0)` という式は短絡評価で `f()` が失敗したその時に限り評価されるということになります。
...
`quote ... end` で囲むと中の式を評価せずに、そのASTを得ることができます。
...
🎉 **つまり、まさに今、自作言語をJuliaにトランスパイルするコードが動きました。** 🎉
...
これだけかければ十分実用的な感じがします。
構文は、元の実装を参考にしてよくある感じの波括弧を使うものにすることにします。

なのでイメージとしては、次のようなコードで FizzBuzz を書けるような言語を目標にします。
