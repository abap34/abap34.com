...
私は次のような事情でテキストベースのもの，とくに marp を好んで使っています．
...
- カスタム hook を作るのが大変．少なくとも marp の記述言語のレベルでレンダプロセスに介入するのは厳しい．
...
さらに重要なこととして，数式ももちろんサポートされているわけですがこのコンパイルはローカルにある (本物の) TeX を呼び出しています．
これによって，あの記号がない，ということは基本的には起きないはずです．


また，レイアウトの制約について，
**制約からのレイアウト生成は完全にこちら側で行い， pdf ライブラリでほぼ「ここにこれを書いてください」** としかしていないという
重要な設計判断をしています．

多くのスライド作成ツールは html へトランスパイルして動作しています．
しかし，当然の話として html 側でユーザが求めているすべてのレイアウトや制約を表現するのは (それ以上に，表現できるかを考えるのが) 結構大変です．
それについて消耗するくらいならこちらでソルバを書いてしまっていくらでも拡張可能にした方がスケールしそうだと考えています．

さらに，補完や型エラーやPage Overflow を含めたエラー検出も備えています．
...
ここで，レイアウトのポリシーは上寄せ，とか中央寄せ，とかそういうのです

### pdf の作成

ここだけはライブラリの使いやすさの問題で Python で書いています．
fpdf2, pypdf を使っています．
